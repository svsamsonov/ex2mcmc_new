...
class RealNVPProposal(nn.Module):
    def __init__(self, lat_size, device, hidden=32, num_blocks=4):
        ...
        self.prior = MultivariateNormal(
            torch.zeros(lat_size).to(device), torch.eye(lat_size).to(device)
        )

        masks = num_blocks * [
            [i % 2 for i in range(lat_size)],
            [(i + 1) % 2 for i in range(lat_size)],
        ]
...
def mala_step(log_target_dens, x0, gamma, mala_iters, stats=None):
    ...
    for i in range(mala_iters):
        ...
        x_cur_proposal = MultivariateNormal(
        ...
        x_next_proposal = MultivariateNormal(
...
def flex2_mcmc(
    ...
    for _ in pbar:
        ...
        if hist_proposals is not None:
            ...
            population_proposals = torch.cat(
                (population_proposals, hist_proposals[idxs])
            )
            population_log_target_dens_proposals = torch.cat(
                (
                    population_log_target_dens_proposals,
                    hist_log_target_dens_proposals[idxs],
                )
            )
        ...
        pbar.set_description(
            f"KL forw {kl_forw.item()}, KL back {kl_back.item()} Hentr {e.item()}"
        )
        ...
        if hist_proposals is None:
            ...
            hist_log_target_dens_proposals = torch.cat(
                (hist_log_target_dens_proposals, log_target_dens_proposals_flattened),
                dim=0,
            )
