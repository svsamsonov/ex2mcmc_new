...
from .ebm_sampling import langevin_sampling, mala_sampling, mh_sampling_normal_proposal
...
def plot_potential_energy(
    ...
    if norm_grads:
        ...
        result = batch_grads_norm.view((num_points, num_points)).detach().cpu().numpy()
        if title is None:
            title = "Latent energy norm gradients"
    else:
        batch_energy = target_energy(batch)
        result = batch_energy.view((num_points, num_points)).detach().cpu().numpy()
        if title is None:
            title = "Latent energy"
...
def epoch_visualization(
    ...
    if path_to_save is not None:
        ...
        if use_gradient_penalty:
            subtitle_for_losses += rf" with gradient penalty, $\lambda = {Lambda}$"
