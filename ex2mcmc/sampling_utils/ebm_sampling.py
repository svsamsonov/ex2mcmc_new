...
def gan_energy(
    ...
    if normalize_to_0_1:
        ...
        gan_part = -(torch.log(sigmoid_gan_part) - torch.log1p(-sigmoid_gan_part)).view(
            -1
        )
...
def xtry_langevin_dynamics(
    ...
    for _ in range(n_steps):
        ...
        X_batch = (
            torch.transpose(X, 0, 1).reshape((batch_size * N, z_dim)).detach().clone()
        )
        ...
        log_gauss = -(X[:, :, None] - T_x[:, None, :]).norm(
            p=2,
            dim=-1,
        ) ** 2 / (2 * eps_scale**2)
